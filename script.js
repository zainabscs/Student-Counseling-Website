// ===== GSAP Animations =====
    window.addEventListener('DOMContentLoaded', () => {
      ...
      // Hero entrance
      gsap.from('.eyebrow', { y: -12, opacity: 0, duration: .6, ease: 'power2.out' });
      gsap.from('h1', { y: 12, opacity: 0, duration: .8, delay: .1, ease: 'power2.out' });
      gsap.from('.lead', { y: 10, opacity: 0, duration: .7, delay: .2, ease: 'power2.out' });
      gsap.from('.hero-actions > *', { y: 16, opacity: 0, duration: .6, stagger: .08, delay: .25, ease: 'power2.out' });
      gsap.to('.orb.cyan', { x: 12, y: -6, repeat: -1, yoyo: true, duration: 3.2, ease: 'sine.inOut' });
      gsap.to('.orb.green', { x: -10, y: 8, repeat: -1, yoyo: true, duration: 3.6, ease: 'sine.inOut' });
      ...
      // Counters (optional visual flair)
      const counters = document.querySelectorAll('#testimonials .card .row > div > div:first-child');
      counters.forEach((c) => {
        const target = parseInt(c.textContent.replace(/[^0-9]/g, ''));
        if (!isNaN(target)) {
      ...
      document.querySelectorAll('.accordion').forEach(acc => {
        ...
        head.addEventListener('click', () => {
          ...
          if (!isOpen) { body.style.display = 'block'; head.querySelector('span').textContent = '–'; }
        });
      });
      ...
      // Simple form handler (prevent real submit in static demo)
      const form = document.getElementById('contact-form');
      form.addEventListener('submit', (e) => {
        e.preventDefault();
        const data = Object.fromEntries(new FormData(form).entries());
        alert(`Thanks ${data.name || 'there'}! We\'ll contact you at ${data.email}.`);
        form.reset();
      });
    });
